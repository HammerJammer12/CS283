...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
	...
	//write student values to student struct
	student.id = id;
...
int del_student(int fd, int id){
	//check if requested student exists
	student_t student;
	int result = get_student(fd, id, &student);
	...
	//student is found, write empty student at that location
...
int count_db_records(int fd){
	...
	//itterate through files records, if record is not empty, increment count
	ssize_t bytesRead;
...
int print_db(int fd){
	...
	while ((bytesRead = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE){
		...
		//print header once when first record is found
		if(!hasRecords){
